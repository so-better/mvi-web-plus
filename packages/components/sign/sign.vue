...
const init = () => {
    ...
    clear()
    //添加鼠标移动事件
    Dap.event.on(document.documentElement, `mousemove.sign_${uid}`, canvasMouseMove)
    //添加鼠标松开事件
    Dap.event.on(document.documentElement, `mouseup.sign_${uid}`, canvasMouseUp)
}
...
onBeforeUnmount(() => {
    ...
    Dap.event.off(document.documentElement, `mousemove.sign_${uid} mouseup.sign_${uid}`)
})
...
<template>
    <canvas class="mvi-sign" ref="canvasRef" @mousedown="canvasMouseDown" @touchstart="canvasTouchStart"
        @touchmove="canvasTouchMove" @touchend="canvasTouchEnd" @mouseleave="canvasMouseLeave"
