...
<template>
    <div class="mvi-date-chooser" ref="elRef" :class="{ block: block }">
        <div class="mvi-date-chooser-relate" :data-id="`mvi-date-chooser-relate-${uid}`" ref="relateRef"
            @click="clickCalendar">
            <slot></slot>
        </div>
        <Layer :relate="`[data-id='mvi-date-chooser-relate-${uid}']`" v-model="show"
            :placement="layerRealProps.placement" :offset="layerRealProps.offset" :z-index="layerRealProps.zIndex"
            :shadow="layerRealProps.shadow" :border="layerRealProps.border" :animation="layerRealProps.animation"
            :border-color="layerRealProps.borderColor" :timeout="layerRealProps.timeout" :closable="closable"
            :show-triangle="layerRealProps.showTriangle" :width="layerRealProps.width" @showing="layerShow">
            <div class="mvi-date-chooser-layer" ref="panelRef">
                ...
                <div v-else-if="view == 'month'" class="mvi-date-chooser-month-header">
                    ...
                    <div class="mvi-date-chooser-month-center" :class="[type]" @click="goYear">{{ formatShow('year',
        selectedDate) }}</div>
                ...
                <Calendar :view="view" v-model="selectedDate" :month-text="monthText" :week-text="weekText"
                    :start-date="startDate" :end-date="endDate" :non-current-click="false" :active="active" :type="type"
                    @date-click="dateClick" @month-click="monthClick" @year-click="yearClick" ref="calendarRef">
                </Calendar>
            </div>
        </Layer>
    </div>
