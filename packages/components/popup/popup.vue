...
<template>
    <Overlay ref="overlayRef" v-model="show" @show="overlayShow" @hide="overlayHide" :use-padding="usePadding"
        :z-index="zIndex" :color="overlayColor" :timeout="timeout" :mount-el="mountEl" :closable="closable">
        <transition :name="'mvi-slide-' + placement" @before-enter="beforeEnter" @enter="enter"
            @after-enter="afterEnter" @before-leave="beforeLeave" @leave="leave" @after-leave="afterLeave">
            <!-- 弹出层 -->
            <div v-if="firstShow" v-show="popupShow" class="mvi-popup" :class="popupClass" :style="popupStyle"
                v-bind="$attrs">
                <!-- 关闭图标 -->
