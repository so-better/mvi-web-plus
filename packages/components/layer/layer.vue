...
<template>
    <teleport to="body">
        <transition :name="animation || 'mvi-layer'" @before-enter="beforeEnter" @enter="enter"
            ...
            <div v-if="firstShow" v-show="layerShow" ref="layerRef" class="mvi-layer" :style="layerStyle"
                v-bind="$attrs">
                <div class="mvi-layer-wrapper" :class="{ shadow: shadow, border: border }" :style="wrapperStyle">
                    <Triangle v-if="showTriangle" ref="triangleRef" class="mvi-layer-triangle"
                        :placement="trianglePlacement" :background="background"
                        :border-color="border && borderColor ? borderColor : background" size="0.14rem"></Triangle>
                    <slot></slot>
                </div>
            </div>
        </transition>
