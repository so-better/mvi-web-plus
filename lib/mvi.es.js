...
const element$1 = {
  ...
  children(el2, selector) {
    ...
    const res = el2.querySelectorAll(selector || "*");
    return [...res].filter((ele) => {
      return ele.parentNode === el2;
    });
  },
  siblings(el2, selector) {
    if (!this.isElement(el2)) {
      throw new TypeError("The first argument must be an element");
    }
    if (selector && typeof selector != "string") {
    ...
    const res = el2.parentNode.querySelectorAll(selector || "*");
    return [...res].filter((ele) => {
      return ele.parentNode === el2.parentNode && ele != el2;
    });
  },
  rem2px(num) {
    if (!number$1.isNumber(num)) {
  ...
  width(el2) {
    if (typeof el2 == "string" && el2) {
      el2 = document.body.querySelector(el2);
    }
    if (!this.isElement(el2)) {
      el2 = document.body;
    }
    let clientWidth = el2.clientWidth;
    let paddingLeft_width = parseFloat(this.getCssStyle(el2, "padding-left"));
    let paddingRight_width = parseFloat(this.getCssStyle(el2, "padding-right"));
  ...
  height(el2) {
    ...
    let clientHeight = el2.clientHeight;
    let paddingTop_height = parseFloat(this.getCssStyle(el2, "padding-top"));
    let paddingBottom_height = parseFloat(this.getCssStyle(el2, "padding-bottom"));
  ...
  scrollTopBottomTrigger(el2, callback) {
    if (typeof el2 == "string" && el2) {
      el2 = document.body.querySelector(el2);
    }
    let scrollEle = window;
    if (this.isElement(el2) && el2 != document.body && el2 != document.documentElement) {
      scrollEle = el2;
    }
  ...
  getScrollWidth(el2) {
    ...
    let scrollWidth = 0;
    if (this.isElement(el2) && el2 != document.documentElement && el2 != document.body) {
  ...
  setScrollTop(options) {
    ...
    if (!this.isElement(el2) || el2 == document.body || el2 == document.documentElement || el2 == window) {
      isWindow = true;
    }
    return new Promise((resolve, reject) => {
      if (time <= 0) {
        if (isWindow) {
          ...
          el2.scrollTop = number$1$1;
        }
        resolve();
      } else {
        let spacingTime = 10;
        let spacingIndex = number$1.divide(time, spacingTime);
        let nowTop = this.getScrollTop(el2);
        ...
        let scrollTimer = setInterval(() => {
          if (spacingIndex > 0) {
            ...
            if (isWindow) {
              ...
              el2.scrollTop = nowTop = number$1.add(nowTop, everTop);
            }
          } else {
    ...
    return new Promise((resolve, reject) => {
      if (time <= 0) {
        if (isWindow) {
          ...
          el2.scrollLeft = number$1$1;
        }
        resolve();
      } else {
        let spacingTime = 10;
        let spacingIndex = number$1.divide(time, spacingTime);
        let nowLeft = this.getScrollLeft(el2);
        ...
        let scrollTimer = setInterval(() => {
          if (spacingIndex > 0) {
            ...
            if (isWindow) {
              ...
              el2.scrollLeft = nowLeft = number$1.add(nowLeft, everLeft);
            }
          } else {
            clearInterval(scrollTimer);
            resolve();
  ...
  getCssStyle(el2, cssName) {
    ...
    if (document.defaultView && document.defaultView.getComputedStyle) {
      cssText = document.defaultView.getComputedStyle(el2)[cssName];
    } else {
      cssText = el2.currentStyle[cssName];
    }
    return cssText;
  },
  ...
  getElementBounding(el2) {
    ...
    let point = el2.getBoundingClientRect();
    let top = point.top;
  ...
  isElement(el2) {
    return el2 && el2 instanceof Node && el2.nodeType === 1;
  },
...
const data$1 = {
  remove(el2, key) {
    if (!(el2 instanceof Document) && !element$1.isElement(el2) && !element$1.isWindow(el2)) {
      throw new TypeError("The first argument must be an element node or window or document");
    }
    let data2 = el2[dataName$1] || {};
    if (key === void 0 || key === null || key === "") {
      el2[dataName$1] = {};
    } else {
      delete data2[key];
      el2[dataName$1] = data2;
    }
  },
  has(el2, key) {
    if (!(el2 instanceof Document) && !element$1.isElement(el2) && !element$1.isWindow(el2)) {
...
const bindSingleListener$1 = (el2, eventName, guid, fn, options) => {
  let events = data$1.get(el2, "dap-defined-events") || {};
  if (!guid) {
    guid = data$1.get(el2, "dap-event-guid") || 0;
    data$1.set(el2, "dap-event-guid", guid + 1);
  }
  guid = eventName + "." + guid;
  if (events[guid] && events[guid].type == eventName) {
    el2.removeEventListener(eventName, events[guid].fn, events[guid].options);
  }
  el2.addEventListener(eventName, fn, options);
  events[guid] = {
    type: eventName,
    fn,
    options
  };
  data$1.set(el2, "dap-defined-events", events);
};
const unbindSingleListener$1 = (el2, eventName, guid) => {
  let events = data$1.get(el2, "dap-defined-events") || {};
  let keys = Object.keys(events);
    ...
    if (events[key].type == eventName) {
      if (guid) {
        if (key == eventName + "." + guid) {
          el2.removeEventListener(events[key].type, events[key].fn, events[key].options);
          events[key] = void 0;
        }
      } else {
        el2.removeEventListener(events[key].type, events[key].fn, events[key].options);
...
const event$1 = {
  on(el2, eventName, fn, options) {
    ...
    result.forEach((res) => {
      bindSingleListener$1(el2, res.eventName, res.guid, fn.bind(el2), options);
    });
  },
  off(el2, eventName) {
    if (!(el2 instanceof Document) && !element$1.isElement(el2) && !element$1.isWindow(el2)) {
    ...
    if (!eventName) {
      ...
      data$1.remove(el2, "dap-defined-events");
      data$1.remove(el2, "dap-event-guid");
      return;
    }
    const result = parseEventName$1(eventName);
    result.forEach((res) => {
      unbindSingleListener$1(el2, res.eventName, res.guid);
    });
  },
  get(el2) {
    if (!(el2 instanceof Document) && !element$1.isElement(el2) && !element$1.isWindow(el2)) {
...
Anchor$1.install = (app) => {
  app.directive("anchor", {
    mounted(el2, binding) {
      ...
      let anchor = new Anchor$1(el2, options);
      anchor.init();
    }
  });
};
...
Resize$1.install = (app) => {
  app.directive("resize", {
    mounted(el2, binding) {
      ...
      let resize = new Resize$1(el2, options);
      resize.init();
      obj$1.data.set(el2, "directive:resize", resize);
    },
    beforeUnmount(el2) {
      let resize = obj$1.data.get(el2, "directive:resize");
      if (resize) {
        resize.destroy();
      }
    }
...
Drag$1.install = (app) => {
  app.directive("drag", {
    mounted(el2, binding) {
      ...
      let drag = new Drag$1(el2, options);
      drag.init();
      obj$1.data.set(el2, "directive:drag", drag);
    },
    beforeUnmount(el2) {
      let drag = obj$1.data.get(el2, "directive:drag");
      if (drag) {
        drag.destroy();
      }
    }
...
Prop$1.install = (app) => {
  app.directive("prop", {
    mounted(el2, binding) {
      let prop = new Prop$1(el2, binding.value);
      prop.init();
      obj$1.data.set(el2, "directive:prop", prop);
    },
    updated(el2, binding) {
      let prop = obj$1.data.get(el2, "directive:prop");
      if (prop) {
...
Scroll$1.install = (app) => {
  app.directive("scroll", {
    mounted(el2, binding) {
      ...
      let scroll = new Scroll$1(el2, options);
      scroll.init();
    }
  });
};
...
Spy$1.install = (app) => {
  app.directive("spy", {
    mounted(el2, binding) {
      ...
      let spy = new Spy$1(el2, options);
      spy.init();
      obj$1.data.set(el2, "directive:spy", spy);
    },
    beforeUnmount(el2) {
      let spy = obj$1.data.get(el2, "directive:spy");
      if (spy) {
        spy.destroy();
      }
    }
...
Observe$1.install = (app) => {
  app.directive("observe", {
    mounted(el2, binding) {
      ...
      let observe = new Observe$1(el2, options);
      observe.init();
    }
  });
};
...
Upload$1.install = (app) => {
  app.directive("upload", {
    mounted(el2, binding) {
      ...
      let upload = new Upload$1(el2, options);
      upload.init();
    }
  });
};
...
class Ripple {
  ...
  endDeal(rippleContainer) {
    ...
    if (rippleUp && rippleAnimationEnd) {
      ...
      setTimeout(() => {
        this.rippleContainers = this.rippleContainers.filter((el2) => {
          return el2 != rippleContainer;
        });
  ...
  createRippleElement(pageX, pageY) {
    ...
    const el2 = obj$1.element.string2dom("<div></div>");
    el2.style.width = r * 2 + "px";
    el2.style.height = r * 2 + "px";
    el2.style.position = "absolute";
    el2.style.background = this.color;
    el2.style.top = y1 - r + "px";
    el2.style.left = x1 - r + "px";
    el2.style.zIndex = 1;
    el2.style.borderRadius = "50%";
    el2.style.transform = "scale(0)";
    el2.style.opacity = this.initialOpacity;
    el2.style.transition = `transform ${this.duration}ms cubic-bezier(0, 0.5, 0.25, 1), opacity ${this.duration}ms cubic-bezier(0.0, 0, 0.2, 1)`;
    return el2;
  }
  createRippleContainer() {
    ...
    el2.style.left = "0px";
    el2.style.top = "0px";
    el2.style.width = obj$1.element.getCssStyle(this.$el, "width");
    el2.style.height = obj$1.element.getCssStyle(this.$el, "height");
    el2.style.background = "transparent";
    el2.style.borderRadius = obj$1.element.getCssStyle(this.$el, "border-radius");
...
Ripple$1.install = (app) => {
  app.directive("ripple", {
    mounted(el2, binding) {
      ...
      let ripple = new Ripple$1(el2, options);
      ripple.init();
      obj$1.data.set(el2, "directive:ripple", ripple);
    },
    beforeUnmount(el2) {
      let ripple = obj$1.data.get(el2, "directive:ripple");
      if (ripple) {
        ripple.destroy();
      }
    }
...
const _sfc_main$_ = {
  ...
  methods: {
    beforeEnter(el2) {
      if (this.fade) {
        el2.style.transition = "opacity " + this.timeout + "ms";
        el2.style.webkitTransition = "opacity " + this.timeout + "ms";
      }
      this.$emit("show", el2);
      if (typeof this.overlayComponentWatch == "function") {
        this.overlayComponentWatch.apply(this, ["show", el2]);
      }
    },
    enter(el2) {
      ...
      this.$emit("showing", el2);
      if (typeof this.overlayComponentWatch == "function") {
        this.overlayComponentWatch.apply(this, ["showing", el2]);
      }
    },
    afterEnter(el2) {
      if (this.fade) {
        el2.style.transition = "";
        el2.style.webkitTransition = "";
      }
      this.$emit("shown", el2);
      if (typeof this.overlayComponentWatch == "function") {
        this.overlayComponentWatch.apply(this, ["shown", el2]);
      }
    },
    beforeLeave(el2) {
      if (this.fade) {
      ...
      this.$emit("hide", el2);
      if (typeof this.overlayComponentWatch == "function") {
        this.overlayComponentWatch.apply(this, ["hide", el2]);
      }
    },
    leave(el2) {
      this.$emit("hidding", el2);
      if (typeof this.overlayComponentWatch == "function") {
        this.overlayComponentWatch.apply(this, ["hidding", el2]);
      }
    },
    afterLeave(el2) {
      ...
      this.$emit("hidden", el2);
      if (typeof this.overlayComponentWatch == "function") {
        this.overlayComponentWatch.apply(this, ["hidden", el2]);
      }
    },
    closeOverlay() {
      if (this.closable) {
...
const _sfc_main$Z = {
  ...
  methods: {
    ...
    beforeEnter(el2) {
      if (obj$1.data.get(el2, "mvi-popup-beforeEnter-trigger")) {
        return;
      }
      obj$1.data.set(el2, "mvi-popup-beforeEnter-trigger", true);
      this.$emit("show", el2);
      if (typeof this.popupComponentWatch == "function") {
        this.popupComponentWatch.apply(this, ["show", el2]);
      }
    },
    enter(el2) {
      if (obj$1.data.get(el2, "mvi-popup-enter-trigger")) {
        return;
      }
      obj$1.data.set(el2, "mvi-popup-enter-trigger", true);
      this.$emit("showing", el2);
      if (typeof this.popupComponentWatch == "function") {
        this.popupComponentWatch.apply(this, ["showing", el2]);
      }
    },
    afterEnter(el2) {
      this.$emit("shown", el2);
      if (typeof this.popupComponentWatch == "function") {
        this.popupComponentWatch.apply(this, ["shown", el2]);
      }
    },
    beforeLeave(el2) {
      obj$1.data.remove(el2, "mvi-popup-beforeEnter-trigger");
      obj$1.data.remove(el2, "mvi-popup-enter-trigger");
      this.$emit("hide", el2);
      if (typeof this.popupComponentWatch == "function") {
        this.popupComponentWatch.apply(this, ["hide", el2]);
      }
    },
    leave(el2) {
      this.$emit("hidding", el2);
      if (typeof this.popupComponentWatch == "function") {
        this.popupComponentWatch.apply(this, ["hidding", el2]);
      }
    },
    afterLeave(el2) {
      this.$emit("hidden", el2);
      if (typeof this.popupComponentWatch == "function") {
        this.popupComponentWatch.apply(this, ["hidden", el2]);
      }
    }
  }
};
    ...
    !$props.loading ? (openBlock(), createElementBlock("div", {
      ...
      (openBlock(true), createElementBlock(Fragment, null, renderList($options.cmpOptions, (column, index) => {
        return openBlock(), createElementBlock("div", {
          ...
          ref_for: true,
          ref: (el2) => $data.itemRefs[index] = el2,
...
const _sfc_main$S = {
  ...
  methods: {
    ...
    doClear() {
      ...
      setTimeout(() => {
        this.realValue = "";
        let el2 = this.$refs.input || this.$refs.textarea;
        el2.focus();
...
const _sfc_main$R = {
  ...
  methods: {
    ...
    beforeEnter(el2) {
      if (obj$1.data.get(el2, "mvi-modal-beforeEnter-trigger")) {
        return;
      }
      obj$1.data.set(el2, "mvi-modal-beforeEnter-trigger", true);
      this.$emit("show", el2);
      if (typeof this.modalComponentWatch == "function") {
        this.modalComponentWatch.apply(this, ["show", el2]);
      }
    },
    enter(el2) {
      if (obj$1.data.get(el2, "mvi-modal-enter-trigger")) {
        return;
      }
      obj$1.data.set(el2, "mvi-modal-enter-trigger", true);
      this.modalSize();
      this.$emit("showing", el2);
      if (typeof this.modalComponentWatch == "function") {
        this.modalComponentWatch.apply(this, ["showing", el2]);
      }
    },
    afterEnter(el2) {
      this.$emit("shown", el2);
      if (typeof this.modalComponentWatch == "function") {
        this.modalComponentWatch.apply(this, ["shown", el2]);
      }
    },
    beforeLeave(el2) {
      obj$1.data.remove(el2, "mvi-modal-beforeEnter-trigger");
      obj$1.data.remove(el2, "mvi-modal-enter-trigger");
      this.$emit("hide", el2);
      if (typeof this.modalComponentWatch == "function") {
        this.modalComponentWatch.apply(this, ["hide", el2]);
      }
    },
    leave(el2) {
      this.$emit("hidding", el2);
      if (typeof this.modalComponentWatch == "function") {
        this.modalComponentWatch.apply(this, ["hidding", el2]);
      }
    },
    afterLeave(el2) {
      this.$emit("hidden", el2);
      if (typeof this.modalComponentWatch == "function") {
        this.modalComponentWatch.apply(this, ["hidden", el2]);
      }
    }
  }
};
  ...
  methods: {
    ...
    modalHide(el2) {
      if (typeof this.dialogComponentWatch == "function") {
        this.dialogComponentWatch.apply(this, ["hide", this.type, el2]);
      }
    },
    modalHidding(el2) {
      if (typeof this.dialogComponentWatch == "function") {
        this.dialogComponentWatch.apply(this, ["hidding", this.type, el2]);
      }
    },
    modalHidden(el2) {
      if (typeof this.dialogComponentWatch == "function") {
        this.dialogComponentWatch.apply(this, ["hidden", this.type, el2]);
      }
      if (this.type == "alert") {
        this.__remove();
    ...
    modalShow(el2) {
      if (typeof this.dialogComponentWatch == "function") {
        this.dialogComponentWatch.apply(this, ["show", this.type, el2]);
      }
    },
    modalShowing(el2) {
      if (typeof this.dialogComponentWatch == "function") {
        this.dialogComponentWatch.apply(this, ["showing", this.type, el2]);
      }
    },
    modalShown(el2) {
      ...
      if (typeof this.dialogComponentWatch == "function") {
        this.dialogComponentWatch.apply(this, ["shown", this.type, el2]);
      }
    }
  }
};
...
const _sfc_main$M = {
  ...
  methods: {
    ...
    beforeEnter(el2) {
      if (obj$1.data.get(el2, "mvi-keyboard-beforeEnter-trigger")) {
        return;
      }
      obj$1.data.set(el2, "mvi-keyboard-beforeEnter-trigger", true);
      this.$emit("show", el2);
      if (typeof this.keyboardComponentWatch == "function") {
        this.keyboardComponentWatch.apply(this, ["show", el2]);
      }
    },
    enter(el2) {
      if (obj$1.data.get(el2, "mvi-keyboard-enter-trigger")) {
        return;
      }
      obj$1.data.set(el2, "mvi-keyboard-enter-trigger", true);
      this.$emit("showing", el2);
      if (typeof this.keyboardComponentWatch == "function") {
        this.keyboardComponentWatch.apply(this, ["showing", el2]);
      }
    },
    afterEnter(el2) {
      this.$emit("shown", el2);
      if (typeof this.keyboardComponentWatch == "function") {
        this.keyboardComponentWatch.apply(this, ["shown", el2]);
      }
    },
    beforeLeave(el2) {
      obj$1.data.remove(el2, "mvi-keyboard-beforeEnter-trigger");
      obj$1.data.remove(el2, "mvi-keyboard-enter-trigger");
      this.$emit("hide", el2);
      if (typeof this.keyboardComponentWatch == "function") {
        this.keyboardComponentWatch.apply(this, ["hide", el2]);
      }
    },
    leave(el2) {
      this.$emit("hidding", el2);
      if (typeof this.keyboardComponentWatch == "function") {
        this.keyboardComponentWatch.apply(this, ["hidding", el2]);
      }
    },
    afterLeave(el2) {
      this.$emit("hidden", el2);
      if (typeof this.keyboardComponentWatch == "function") {
        this.keyboardComponentWatch.apply(this, ["hidden", el2]);
      }
    }
  }
};
...
const _sfc_main$J = {
  ...
  methods: {
    onDrag(el2, container, placement) {
      this.isdrag = true;
      if (this.vertical) {
...
const _sfc_main$D = {
  ...
  methods: {
    ...
    onPull(event2) {
      ...
      let el2 = this.getScrollEl(event2.target);
      if (!el2.isEqualNode(this.$refs.wrapper) && obj$1.element.getScrollTop(el2) > 0) {
        this.firstStartY = this.startY;
        return;
      }
      this.hasTouch = true;
    ...
    getScrollEl(el2) {
      if (el2.isEqualNode(this.$refs.wrapper) || !obj$1.element.isContains(this.$refs.wrapper, el2)) {
        return this.$refs.wrapper;
      }
      if (obj$1.element.getScrollHeight(el2) > el2.clientHeight) {
        return el2;
      }
      return this.getScrollEl(el2.parentNode);
    }
  },
  beforeUnmount() {
...
const _sfc_main$z = {
  ...
  methods: {
    beforeEnter(el2) {
      el2.style.height = 0;
      el2.style.transition = "height " + this.timeout + "ms linear";
      el2.style.webkitTransition = "height " + this.timeout + "ms linear";
      this.$emit("before-slide-down", this.$el);
    },
    enter(el2) {
      el2.offsetWidth;
      el2.style.height = this.height;
    },
    afterEnter(el2) {
      el2.style.transition = "";
      el2.style.webkitTransition = "";
    ...
    beforeLeave(el2) {
      if (!this.height) {
        this.height = obj$1.element.getCssStyle(el2, "height");
      }
      el2.style.height = this.height;
  ...
  methods: {
    addTransition(el2) {
      if (this.swiper.useOpacity) {
        el2.style.transition = "opacity " + this.swiper.speed + "ms";
        el2.style.webkitTransition = "opacity " + this.swiper.speed + "ms";
      } else {
        el2.style.transition = "";
        el2.style.webkitTransition = "";
      }
    },
    removeTransition(el2) {
      if (this.swiper.useOpacity) {
        el2.style.transition = "";
        el2.style.webkitTransition = "";
      } else {
...
const tabs_vue_vue_type_style_index_0_scoped_f6c13528_lang = "";
const _sfc_main$q = {
  name: "m-tabs",
  data() {
    return {
...
const Tabs = /* @__PURE__ */ _export_sfc(_sfc_main$q, [["render", _sfc_render$q], ["__scopeId", "data-v-f6c13528"]]);
Tabs.install = (app) => {
...
const _sfc_main$n = {
  ...
  methods: {
    ...
    beforeEnter(el2) {
      if (obj$1.data.get(el2, "mvi-layer-beforeEnter-trigger")) {
        return;
      }
      obj$1.data.set(el2, "mvi-layer-beforeEnter-trigger", true);
      this.$emit("show", el2);
      if (typeof this.layerComponentWatch == "function") {
        this.layerComponentWatch.apply(this, ["show", el2]);
      }
    },
    enter(el2) {
      if (obj$1.data.get(el2, "mvi-layer-enter-trigger")) {
        return;
      }
      obj$1.data.set(el2, "mvi-layer-enter-trigger", true);
      this.$nextTick(() => {
        this.autoAdjust();
        this.reset();
        this.$nextTick(() => {
        ...
        this.$emit("showing", el2);
        if (typeof this.layerComponentWatch == "function") {
          this.layerComponentWatch.apply(this, ["showing", el2]);
        }
      });
    },
    afterEnter(el2) {
      this.$emit("shown", el2);
      if (typeof this.layerComponentWatch == "function") {
        this.layerComponentWatch.apply(this, ["shown", el2]);
      }
    },
    beforeLeave(el2) {
      obj$1.data.remove(el2, "mvi-layer-beforeEnter-trigger");
      obj$1.data.remove(el2, "mvi-layer-enter-trigger");
      this.$emit("hide", el2);
      if (typeof this.layerComponentWatch == "function") {
        this.layerComponentWatch.apply(this, ["hide", el2]);
      }
    },
    leave(el2) {
      this.$emit("hidding", el2);
      if (typeof this.layerComponentWatch == "function") {
        this.layerComponentWatch.apply(this, ["hidding", el2]);
      }
    },
    afterLeave(el2) {
      this.$emit("hidden", el2);
      if (typeof this.layerComponentWatch == "function") {
        this.layerComponentWatch.apply(this, ["hidden", el2]);
      }
    },
    reset() {
      if (!obj$1.element.isElement(this.$el)) {
...
const table_vue_vue_type_style_index_0_scoped_ee8ba653_lang = "";
const _sfc_main$l = {
  name: "m-table",
  ...
  computed: {
    ...
    bodyColumnStyle() {
      return (column, index) => {
        this.columnAlignKey;
        return {
          width: this.headerColumnRefs[index] ? obj$1.element.getCssStyle(this.headerColumnRefs[index], "width") : "0px"
        };
      };
    },
    sortIconStyle() {
...
const _hoisted_3$c = { ref: "headerRow" };
const _hoisted_4$a = {
  key: 0,
...
function _sfc_render$l(_ctx, _cache, $props, $setup, $data, $options) {
  ...
  return openBlock(), createElementBlock("div", {
            ...
            return openBlock(), createElementBlock("th", {
              ...
              createElementVNode("div", {
                ...
                column.type == "selection" ? (openBlock(), createBlock(_component_Checkbox, {
                  ...
                  column.sortable ? (openBlock(), createElementBlock("span", _hoisted_4$a, [
                    createVNode(_component_Icon, {
                      type: "caret-up",
    ...
    createElementVNode("div", {
      ...
      $props.loading ? (openBlock(), createElementBlock("div", _hoisted_5$9, [
        createVNode(_component_Loading, {
          color: "#bbb",
          size: "0.3rem"
        }),
        ...
        $data.rowData.length ? (openBlock(), createElementBlock("table", _hoisted_6$8, [
                  ...
                  createVNode(_component_Tooltip, {
                    ...
                    default: withCtx(() => [
                      createElementVNode("div", {
                        ...
                        }, null, 8, ["modelValue", "value", "onChange", "color", "disabled"])) : (openBlock(), createElementBlock("div", {
                          key: 1,
                          class: normalizeClass([column.ellipsis ? "ellipsis" : ""])
                        }, [
                          column.type == "custom" && _ctx.$slots.custom ? renderSlot(_ctx.$slots, "custom", {
                            key: 0,
                            row,
                            rowIndex,
                            column,
...
const Table = /* @__PURE__ */ _export_sfc(_sfc_main$l, [["render", _sfc_render$l], ["__scopeId", "data-v-ee8ba653"]]);
Table.install = (app) => {
  ...
  return openBlock(), createElementBlock("div", _hoisted_1$c, [
    $props.view == "year" ? (openBlock(true), createElementBlock(Fragment, { key: 0 }, renderList(new Array(3), (item, index) => {
      return openBlock(), createElementBlock("div", _hoisted_2$c, [
        (openBlock(true), createElementBlock(Fragment, null, renderList($options.years.slice(index * 4, index * 4 + 4), (el2) => {
          return openBlock(), createElementBlock("div", _hoisted_3$b, [
            createElementVNode("div", {
              class: normalizeClass($options.yearClass(el2)),
              disabled: $options.yearDisabled(el2) || null,
              onClick: ($event) => $options.yearClick(el2)
            }, toDisplayString(el2.getFullYear()), 11, _hoisted_4$9)
          ]);
        }), 256))
      ]);
      ...
      return openBlock(), createElementBlock("div", _hoisted_5$8, [
        (openBlock(true), createElementBlock(Fragment, null, renderList($options.months.slice(index * 4, index * 4 + 4), (el2) => {
          return openBlock(), createElementBlock("div", _hoisted_6$7, [
            createElementVNode("div", {
              class: normalizeClass($options.monthClass(el2)),
              disabled: $options.monthDisabled(el2) || null,
              onClick: ($event) => $options.monthClick(el2)
            }, toDisplayString($props.monthText[el2.getMonth()]), 11, _hoisted_7$6)
          ]);
        }), 256))
      ]);
      ...
      createElementVNode("div", _hoisted_11$3, [
        (openBlock(true), createElementBlock(Fragment, null, renderList(new Array(6), (item, index) => {
          return openBlock(), createElementBlock("div", _hoisted_12$2, [
            (openBlock(true), createElementBlock(Fragment, null, renderList($options.days.slice(index * 7, index * 7 + 7), (el2, i) => {
              return openBlock(), createElementBlock("div", _hoisted_13$2, [
                createElementVNode("div", {
                  class: normalizeClass($options.dateClass(el2)),
                  disabled: $options.dateDisabled(el2) || null,
                  onClick: ($event) => $options.dateClick(el2)
                }, toDisplayString(el2.date.getDate()), 11, _hoisted_14$2)
              ]);
            }), 256))
          ]);
        }), 256))
            ...
            return openBlock(), createBlock(_component_SwiperSlide, { class: "mvi-preview-container" }, {
              default: withCtx(() => [
                createVNode(_component_RichImage, {
                  ref_for: true,
                  ref: (el2) => $data.imageRefs[index] = el2,
...
const page_vue_vue_type_style_index_0_scoped_ffcb1a13_lang = "";
const _sfc_main$g = {
  name: "m-page",
  data() {
    return {
...
const Page = /* @__PURE__ */ _export_sfc(_sfc_main$g, [["render", _sfc_render$g], ["__scopeId", "data-v-ffcb1a13"]]);
Page.install = (app) => {
...
const _sfc_main$e = {
  ...
  methods: {
    initScroll() {
      const el2 = document.body.querySelector(this.scrollEl);
      if (!el2) {
        return;
      }
      if (this.scrollObj) {
      ...
      this.scrollObj = new Scroll$1(el2, {
        bottom: () => {
        ...
        let scrollTimer = setInterval(() => {
          if (spacingIndex > 0) {
            ...
            if (isWindow) {
              ...
              el2.scrollTop = nowTop = number.add(nowTop, everTop);
            }
          } else {
            clearInterval(scrollTimer);
            resolve();
        ...
        let scrollTimer = setInterval(() => {
          if (spacingIndex > 0) {
            ...
            if (isWindow) {
              ...
              el2.scrollLeft = nowLeft = number.add(nowLeft, everLeft);
            }
          } else {
            clearInterval(scrollTimer);
            resolve();
...
const data = {
  remove(el2, key) {
    if (!(el2 instanceof Document) && !element.isElement(el2) && !element.isWindow(el2)) {
      throw new TypeError("The first argument must be an element node or window or document");
    }
    let data2 = el2[dataName] || {};
    if (key === void 0 || key === null || key === "") {
      el2[dataName] = {};
    } else {
      delete data2[key];
...
const common = {
  ...
  copyText(text) {
    ...
    let el2 = element.string2dom('<input type="text" />');
...
const bindSingleListener = (el2, eventName, guid, fn, options) => {
  let events = data.get(el2, "dap-defined-events") || {};
  if (!guid) {
    guid = data.get(el2, "dap-event-guid") || 0;
    data.set(el2, "dap-event-guid", guid + 1);
  }
  guid = eventName + "." + guid;
  if (events[guid] && events[guid].type == eventName) {
    el2.removeEventListener(eventName, events[guid].fn, events[guid].options);
  ...
  data.set(el2, "dap-defined-events", events);
};
const unbindSingleListener = (el2, eventName, guid) => {
  let events = data.get(el2, "dap-defined-events") || {};
  let keys = Object.keys(events);
  let length = keys.length;
  for (let i = 0; i < length; i++) {
...
const event = {
  on(el2, eventName, fn, options) {
    ...
    result.forEach((res) => {
      bindSingleListener(el2, res.eventName, res.guid, fn.bind(el2), options);
    });
  },
  off(el2, eventName) {
    if (!(el2 instanceof Document) && !element.isElement(el2) && !element.isWindow(el2)) {
    ...
    if (!eventName) {
      ...
      data.remove(el2, "dap-defined-events");
      data.remove(el2, "dap-event-guid");
      return;
    }
    const result = parseEventName(eventName);
    result.forEach((res) => {
      unbindSingleListener(el2, res.eventName, res.guid);
    });
  },
  get(el2) {
    if (!(el2 instanceof Document) && !element.isElement(el2) && !element.isWindow(el2)) {
...
const Util = {
  getAttributes(el2) {
    let o = {};
    const length = el2.attributes.length;
    for (let i = 0; i < length; i++) {
      const attribute = el2.attributes[i];
  ...
  getStyles(el2) {
    let o = {};
    if (el2.getAttribute("style")) {
      const styles = el2.getAttribute("style");
      let i = 0;
      let start = 0;
      let splitStyles = [];
...
const _AlexElement = class {
  ...
  isEmpty() {
    ...
    if (this.isBlock() || this.isInblock() || this.isInline()) {
      ...
      const allEmpty = this.children.every((el2) => {
        return !el2 || el2.isEmpty();
      });
      return allEmpty;
    }
    return false;
  ...
  clone(deep = true) {
    ...
    let el2 = new _AlexElement(this.type, this.parsedom, Util.clone(this.marks), Util.clone(this.styles), this.textContent);
    el2.behavior = this.behavior;
    if (deep && this.hasChildren()) {
      this.children.forEach((child) => {
        let clonedChild = child.clone(deep);
        if (el2.hasChildren()) {
          el2.children.push(clonedChild);
        } else {
          el2.children = [clonedChild];
        }
        clonedChild.parent = el2;
      });
    }
    return el2;
  }
  ...
  __renderElement() {
    let el2 = null;
    if (this.isText()) {
      el2 = document.createElement(_AlexElement.TEXT_NODE);
      const text = document.createTextNode(this.textContent);
      el2.appendChild(text);
    } else {
      el2 = document.createElement(this.parsedom);
      if (this.hasChildren()) {
        this.children.forEach((child) => {
          child.__renderElement();
          el2.appendChild(child._elm);
        });
      }
    }
    if (this.hasMarks()) {
      Object.keys(this.marks).forEach((key) => {
        if (!/(^on)|(^style$)|(^contenteditable$)/g.test(key)) {
          el2.setAttribute(key, this.marks[key]);
        }
      });
    }
    if (this.hasStyles()) {
      Object.keys(this.styles).forEach((key) => {
        el2.style.setProperty(key, this.styles[key]);
      });
    }
    obj.data.set(el2, "data-alex-editor-key", this.key);
    this._elm = el2;
  }
  __fullClone() {
    let el2 = new _AlexElement(this.type, this.parsedom, Util.clone(this.marks), Util.clone(this.styles), this.textContent);
...
class AlexPoint {
  ...
  moveToEnd(element2) {
    ...
    if (element2.isText()) {
      ...
      const flatElements = AlexElement.flatElements(element2.children).filter((el2) => {
        return !el2.isEmpty();
      });
...
class AlexEditor {
    ...
    __publicField(this, "__formatUnchangeableRules", [
      ...
      (element2) => {
        if (element2.hasChildren()) {
          const hasBlock = element2.children.some((el3) => {
            return !el3.isEmpty() && el3.isBlock();
          });
          if (hasBlock) {
            element2.children.forEach((el3) => {
              if (!el3.isEmpty() && el3.isBlock()) {
                el3.type = element2.type == "inline" ? "inline" : "inblock";
              }
            });
          }
        }
      ...
      (element2) => {
        if (element2.hasChildren()) {
          let hasInblock = element2.children.some((el3) => {
            return !el3.isEmpty() && el3.isInblock();
          });
          let hasOtherElement = element2.children.some((el3) => {
            return !el3.isEmpty() && !el3.isInblock();
          });
          if (hasInblock && hasOtherElement) {
            element2.children.forEach((el3) => {
              if (!el3.isEmpty() && el3.isInblock()) {
                el3.type = "inline";
              }
            });
          }
      ...
      (element2) => {
        if (element2.isInline() && element2.hasChildren()) {
          const hasInblock = element2.children.some((el3) => {
            return !el3.isEmpty() && el3.isInblock();
          });
          if (hasInblock) {
            element2.children.forEach((el3) => {
      ...
      (element2) => {
        if (element2.hasChildren()) {
          if (element2.isBlock() || element2.isInblock()) {
            let hasBreak = element2.children.some((el3) => {
              return el3.isBreak();
            });
            let hasOtherElement = element2.children.some((el3) => {
              return !el3.isEmpty() && !el3.isBreak();
            });
            if (hasBreak && hasOtherElement) {
              ...
              element2.children.forEach((el3, index) => {
                if (el3.isBreak() && index > 0) {
                  el3.toEmpty();
                }
              });
            }
  ...
  __setRangeInVisible() {
    ...
    const fn = (root2) => {
      ...
      if (root2.clientHeight < scrollHeight) {
        const el2 = this.range.focus.element._elm;
        const childRect = obj.element.getElementBounding(el2);
        ...
        if (isHiddenTop(childRect.top, parentRect.top) || isHiddenBottom(childRect.bottom, parentRect.bottom)) {
          obj.element.setScrollTop({
            ...
            const tempChildRect = obj.element.getElementBounding(el2);
  ...
  __insertNewDom(el2, reRender = true) {
    if (reRender) {
      el2.__renderElement();
    }
    const previousElement = this.getPreviousElement(el2);
    if (previousElement) {
      previousElement._elm.parentNode.insertBefore(el2._elm, previousElement._elm.nextSibling);
    } else {
      if (el2.isBlock()) {
        if (this.$el.firstElementChild) {
          this.$el.insertBefore(el2._elm, this.$el.firstElementChild);
        } else {
          this.$el.appendChild(el2._elm);
        }
      } else {
        const parent = el2.parent._elm;
        if (parent.firstElementChild) {
          parent.insertBefore(el2._elm, parent.firstElementChild);
        } else {
          parent.appendChild(el2._elm);
        }
      }
    }
  }
    ...
    if (hasHtml) {
      for (let i = 0; i < length; i++) {
        ...
        if (blob.type == "text/plain" && !this.htmlPaste) {
          ...
          const data2 = await blob.text();
          if (data2) {
            const elements = this.parseHtml(data2).filter((el2) => {
              return !el2.isEmpty();
            });
    ...
    const format = (element2, fn) => {
      if (element2.hasChildren()) {
        ...
        while (index < element2.children.length) {
          let el2 = element2.children[index];
          format(el2, fn);
          const newIndex = element2.children.findIndex((item) => {
            return el2.isEqual(item);
          });
          index = newIndex + 1;
        }
      }
  ...
  parseHtml(html) {
    ...
    node.innerHTML = html;
    let elements = [];
    Array.from(node.childNodes).forEach((el2) => {
      if (el2.nodeType == 1 || el2.nodeType == 3) {
        const element2 = this.parseNode(el2);
    ...
    const fn = (elements) => {
      ...
      while (i < length) {
        ...
        if (elements[i].hasChildren()) {
          const el2 = fn(elements[i].children);
          if (el2) {
            element2 = el2;
            break;
          }
  ...
  getElementsByRange(includes = false, flat = false) {
    ...
    const anchorIndex = elements.findIndex((el2) => el2.isEqual(this.range.anchor.element));
    const focusIndex = elements.findIndex((el2) => el2.isEqual(this.range.focus.element));
    ...
    result.forEach((item, index) => {
      if (item.offset) {
        ...
        if (item.offset[0] == 0) {
          ...
          this.addElementAfter(el2, item.element);
          ...
          el2.textContent = el2.textContent.substring(item.offset[0]);
          this.addElementAfter(el2, item.element);
          selectEl = el2;
        } else {
          const el2 = item.element.clone();
          const el22 = item.element.clone();
          item.element.textContent = item.element.textContent.substring(0, item.offset[0]);
          el2.textContent = el2.textContent.substring(item.offset[0], item.offset[1]);
          el22.textContent = el22.textContent.substring(item.offset[1]);
          this.addElementAfter(el2, item.element);
          this.addElementAfter(el22, el2);
          selectEl = el2;
        }
        if (index == 0) {
  ...
  setTextStyle(styles) {
    ...
    if (this.range.anchor.isEqual(this.range.focus)) {
      if (this.range.anchor.element.isSpaceText()) {
        ...
        const el2 = AlexElement.getSpaceElement();
        el2.styles = Util.clone(this.range.anchor.element.styles);
        el2.marks = Util.clone(this.range.anchor.element.marks);
        if (el2.hasStyles()) {
          Object.assign(el2.styles, Util.clone(styles));
        } else {
          el2.styles = Util.clone(styles);
        }
        this.insertElement(el2);
      } else {
        const el2 = AlexElement.getSpaceElement();
        el2.styles = Util.clone(styles);
        this.insertElement(el2);
  ...
  removeTextStyle(styleNames) {
    const removeFn = (el2) => {
      if (Array.isArray(styleNames)) {
        if (el2.hasStyles()) {
          let styles = {};
          Object.keys(el2.styles).forEach((key) => {
            if (!styleNames.includes(key)) {
              styles[key] = el2.styles[key];
            }
          });
          el2.styles = styles;
        }
  ...
  setTextMark(marks) {
    ...
    if (this.range.anchor.isEqual(this.range.focus)) {
      if (this.range.anchor.element.isSpaceText()) {
        ...
        el2.marks = Util.clone(this.range.anchor.element.marks);
        if (el2.hasMarks()) {
          Object.assign(el2.marks, Util.clone(marks));
        } else {
          el2.marks = Util.clone(marks);
        }
        this.insertElement(el2);
      } else {
        const el2 = AlexElement.getSpaceElement();
  ...
  removeTextMark(markNames) {
    const removeFn = (el2) => {
      if (Array.isArray(markNames)) {
        if (el2.hasMarks()) {
          let marks = {};
          Object.keys(el2.marks).forEach((key) => {
            if (!markNames.includes(key)) {
              marks[key] = el2.marks[key];
            }
          });
          el2.marks = marks;
        }
...
const listLanguages = HighlightJS.listLanguages();
...
const editor_vue_vue_type_style_index_0_scoped_2c9742d8_lang = "";
const _sfc_main$7 = {
  name: "m-editor",
  ...
  methods: {
    ...
    autoLayerOffset(refName) {
      const b1 = obj$1.element.getElementBounding(this.$el);
      const b2 = obj$1.element.getElementBounding(this.$refs[refName].$el);
    ...
    onScroll() {
      const setScroll = (el2) => {
        obj$1.event.on(el2, `scroll.editor_${this.uid}`, (e) => {
        ...
        if (el2.parentNode) {
          setScroll(el2.parentNode);
        }
      };
      setScroll(this.$refs.content);
    },
    removeScroll() {
      const removeScroll = (el2) => {
        obj$1.event.off(el2, `scroll.editor_${this.uid}`);
        if (el2.parentNode) {
          removeScroll(el2.parentNode);
        }
      };
    ...
    orderListHandle(element2) {
      if (!element2.isEmpty()) {
        if (element2.parsedom == "ol" || element2.parsedom == "ul") {
          if (element2.hasChildren()) {
            element2.children.forEach((el2, index) => {
              const newEl = el2.clone();
              newEl.parsedom = "div";
              newEl.type = "block";
              if (!newEl.hasMarks()) {
                newEl.marks = {};
    ...
    tableHandle(element2) {
      if (element2.parsedom == "table" && !element2.isEmpty()) {
        ...
        const rows = elements.filter((el2) => {
          return el2.parsedom == "tr";
        });
        let colgroup = elements.find((el2) => {
          return el2.parsedom == "colgroup";
        });
        if (colgroup) {
    ...
    preHandle(element2) {
      if (element2.isPreStyle() && element2.parsedom == "pre" && !element2.isEmpty()) {
        const marks = {
          "mvi-editor-element-key": element2.key
        };
        if (element2.hasMarks()) {
        ...
        if (element2.hasChildren()) {
          ...
          const originalTextElements = AlexElement.flatElements(element2.children).filter((el2) => el2.isText() && !el2.isEmpty());
    ...
    handleRangeUpdate(range) {
      ...
      setTimeout(() => {
        if (img || video) {
          const el2 = img || video;
          this.mediaAdjusterProps.target = `[mvi-editor-element-key='${el2.key}']`;
          this.mediaAdjusterProps.element = el2;
    ...
    handleAfterRender() {
      const elements = AlexElement.flatElements(this.editor.stack);
      const tables = elements.filter((el2) => el2.parsedom == "table");
      tables.forEach((table) => {
        const firstRow = AlexElement.flatElements(table.children).filter((el2) => {
          return el2.parsedom == "tr";
        })[0];
    ...
    handlePasteHtml(data2, elements) {
      AlexElement.flatElements(elements).forEach((el2) => {
        if (["table", "tr", "th", "td", "tbody", "br", "blockquote", "ol", "ul", "li", "pre", "code", "label", "hr", "colgroup"].includes(el2.parsedom)) {
          el2.marks = null;
          if (el2.isBlock() || el2.isInblock()) {
            if (el2.hasStyles()) {
              let styles = {};
              for (let key in el2.styles) {
    ...
    updateRangeInPre(element2, originalTextElements, newElements) {
      if (this.editor.range.anchor.element.getBlock().isEqual(element2)) {
        const elIndex = originalTextElements.findIndex((el2) => this.editor.range.anchor.element.isEqual(el2));
        const offset = originalTextElements.filter((el2, i2) => i2 < elIndex).reduce((total, item, i2) => total + item.textContent.length, 0) + this.editor.range.anchor.offset;
        const newTextElements = AlexElement.flatElements(newElements).filter((el2) => el2.isText() && !el2.isEmpty());
        let i = 0;
        let index = 0;
      ...
      if (this.editor.range.focus.element.getBlock().isEqual(element2)) {
        const elIndex = originalTextElements.findIndex((el2) => this.editor.range.focus.element.isEqual(el2));
        const offset = originalTextElements.filter((el2, i2) => i2 < elIndex).reduce((total, item, i2) => total + item.textContent.length, 0) + this.editor.range.focus.offset;
        const newTextElements = AlexElement.flatElements(newElements).filter((el2) => el2.isText() && !el2.isEmpty());
        let i = 0;
        let index = 0;
        while (i < newTextElements.length) {
    ...
    insertHtml(html) {
      ...
      elements.reverse().forEach((el2, index) => {
        if (index == 0) {
          this.editor.insertElement(el2);
        } else {
          this.editor.addElementBefore(el2, elements[0]);
        }
      });
      this.editor.range.anchor.moveToEnd(elements[elements.length - 1]);
      this.editor.range.focus.moveToEnd(elements[elements.length - 1]);
...
const _hoisted_10$1 = { class: "mvi-editor-layer-item" };
...
const Editor = /* @__PURE__ */ _export_sfc(_sfc_main$7, [["render", _sfc_render$7], ["__scopeId", "data-v-2c9742d8"]]);
Editor.install = (app) => {
...
const editorMenu_vue_vue_type_style_index_0_scoped_5715b1a3_lang = "";
const getMenu = (name) => {
  if (name == "custom") {
    return null;
  }
        ...
        error: (errorType, file2) => {
          if (this.name == "image") {
            if (typeof this.menus.uploadImageError == "function") {
              this.menus.uploadImageError(state, file2);
            } else {
              let msg = "";
    ...
    handleOpt(item) {
      ...
      if (this.name == "undo") {
        ...
        if (editor.range.anchor.isEqual(editor.range.focus)) {
          ...
          result.forEach((el2) => {
            if (el2.element.isBlock()) {
              elementUtil.toParagraph(el2.element);
              el2.element.parsedom = item.value;
            } else {
              const block2 = el2.element.getBlock();
              elementUtil.toParagraph(block2);
              block2.parsedom = item.value;
            }
          });
        ...
        if (editor.range.anchor.isEqual(editor.range.focus)) {
          ...
          const result = editor.getElementsByRange(true, false);
          result.forEach((el2) => {
            if (el2.element.isBlock() || el2.element.isInblock()) {
              if (el2.element.hasStyles()) {
                el2.element.styles["text-align"] = item.value;
              } else {
                el2.element.styles = {
                  "text-align": item.value
                };
              ...
              const block2 = el2.element.getBlock();
              const inblock2 = el2.element.getInblock();
              if (inblock2) {
                if (inblock2.hasStyles()) {
                  inblock2.styles["text-align"] = item.value;
                } else {
        ...
        if (editor.range.anchor.isEqual(editor.range.focus)) {
          ...
          result.forEach((el2) => {
            if (el2.element.isBlock()) {
              elementUtil.toParagraph(el2.element);
              if (!this.active) {
                el2.element.parsedom = "blockquote";
              }
            } else {
              const block2 = el2.element.getBlock();
              elementUtil.toParagraph(block2);
        ...
        if (this.active) {
          ...
          if (editor.range.anchor.isEqual(editor.range.focus)) {
            ...
            editor.addElementAfter(paragraph, block2);
          } else {
            ...
            const res = editor.getElementsByRange(true, true).filter((el2) => el2.element.isText());
            const obj2 = {};
            res.forEach((el2) => {
              if (obj2[el2.element.getBlock().key]) {
                obj2[el2.element.getBlock().key].push(el2.element.clone());
              } else {
                obj2[el2.element.getBlock().key] = [el2.element.clone()];
              }
            });
            ...
            Object.keys(obj2).forEach((key, index) => {
              ...
              obj2[key].forEach((el2) => {
                if (pre.hasChildren()) {
                  editor.addElementTo(el2, pre, pre.children.length);
                } else {
                  editor.addElementTo(el2, pre);
                }
              });
            });
            editor.delete();
      ...
      this.rangeUpdateTimer = setTimeout(() => {
        ...
        if (this.name == "code") {
          ...
          if (editor.range.anchor.isEqual(editor.range.focus)) {
            ...
            const result = editor.getElementsByRange(true, false);
            this.selectedVal = this.parseList.find((item) => {
              return result.every((el2) => {
                if (el2.element.isBlock()) {
                  return el2.element.parsedom == item.value;
                }
                return el2.element.getBlock().parsedom == item.value;
              });
            }) || { ...this.defaultVal };
          }
        } else if (this.name == "fontfamily") {
          ...
          if (editor.range.anchor.isEqual(editor.range.focus)) {
            ...
            this.selectedVal = this.parseList.find((item) => {
              return result.every((el2) => {
                if (el2.element.isBlock() || el2.element.isInblock()) {
                  return el2.element.hasStyles() && el2.element.styles["text-align"] == item.value;
                }
                const block2 = el2.element.getBlock();
                const inblock2 = el2.element.getInblock();
                if (inblock2) {
            ...
            $data.active ? (openBlock(), createElementBlock(Fragment, { key: 0 }, [
              ...
              (openBlock(true), createElementBlock(Fragment, null, renderList($data.tableParams.grids, (item) => {
                return openBlock(), createElementBlock("div", _hoisted_9, [
                  (openBlock(true), createElementBlock(Fragment, null, renderList(item, (el2) => {
                    return openBlock(), createElementBlock("div", {
                      onClick: _cache[6] || (_cache[6] = (...args) => $options.confirmTableSize && $options.confirmTableSize(...args)),
                      onMouseenter: ($event) => $options.changeTableSize(el2),
                      class: normalizeClass(["mvi-editor-menu-table-grid", el2.inside ? "active" : ""])
                    }, null, 42, _hoisted_10);
                  }), 256))
                ]);
              }), 256)),
...
const EditorMenu = /* @__PURE__ */ _export_sfc(_sfc_main$4, [["render", _sfc_render$4], ["__scopeId", "data-v-5715b1a3"]]);
