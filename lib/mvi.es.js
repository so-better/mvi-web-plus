  ...
  computed: {
    ...
    selectLabel() {
      if (this.multiple) {
        ...
        this.options.forEach((item, index) => {
          if (Array.isArray(this.modelValue)) {
            let flag = this.modelValue.some((i) => {
              return obj$1.common.equal(i, item[this.props.value]);
            });
            if (flag) {
        ...
        let label = "";
        this.options.forEach((item, index) => {
          if (obj$1.common.equal(this.modelValue, item[this.props.value])) {
    ...
    optionClick(item) {
      ...
      if (this.multiple) {
        ...
        let flag = arr.some((tmp) => {
          return obj$1.common.equal(tmp, item[this.props.value]);
        });
        if (flag) {
          arr = arr.filter((tmp) => {
            return !obj$1.common.equal(tmp, item[this.props.value]);
        ...
        this.$emit(
          ...
          this.options.filter((tmp) => {
            return arr.some((tmp2) => {
              return obj$1.common.equal(tmp[this.props.value], tmp2);
            });
          })
        );
      } else {
...
class AlexEditor {
  constructor(el, options) {
    __publicField(this, "__formatUnchangeableRules", [
      ...
      (element2) => {
        if (element2.hasChildren()) {
          const hasBlock = element2.children.some((el2) => {
            return !el2.isEmpty() && el2.isBlock();
          });
          if (hasBlock) {
            element2.children.forEach((el2) => {
              if (!el2.isEmpty() && el2.isBlock()) {
      ...
      (element2) => {
        if (element2.hasChildren()) {
          let hasInblock = element2.children.some((el2) => {
            return !el2.isEmpty() && el2.isInblock();
          });
          let hasOtherElement = element2.children.some((el2) => {
            return !el2.isEmpty() && !el2.isInblock();
          });
          if (hasInblock && hasOtherElement) {
            element2.children.forEach((el2) => {
              if (!el2.isEmpty() && el2.isInblock()) {
                el2.type = "inline";
              }
            });
          }
      ...
      (element2) => {
        if (element2.isInline() && element2.hasChildren()) {
          const hasInblock = element2.children.some((el2) => {
            return !el2.isEmpty() && el2.isInblock();
          });
          if (hasInblock) {
            element2.children.forEach((el2) => {
      ...
      (element2) => {
        if (element2.hasChildren()) {
          if (element2.isBlock() || element2.isInblock()) {
            let hasBreak = element2.children.some((el2) => {
              return el2.isBreak();
            });
            let hasOtherElement = element2.children.some((el2) => {
              return !el2.isEmpty() && !el2.isBreak();
            });
...
const editor_vue_vue_type_style_index_0_scoped_95ea994b_lang = "";
const _sfc_main$7 = {
  name: "m-editor",
...
const Editor = /* @__PURE__ */ _export_sfc(_sfc_main$7, [["render", _sfc_render$7], ["__scopeId", "data-v-95ea994b"]]);
Editor.install = (app) => {
...
const editorMenu_vue_vue_type_style_index_0_scoped_c073c6f1_lang = "";
const getMenu = (name) => {
  if (name == "custom") {
    return null;
  }
...
const EditorMenu = /* @__PURE__ */ _export_sfc(_sfc_main$4, [["render", _sfc_render$4], ["__scopeId", "data-v-c073c6f1"]]);
